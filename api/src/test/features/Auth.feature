...
Feature: Authentication API
  ...
  Scenario: Attempt to authenticate with invalid credentials
    ...
    And the user has username "admin11" and password "password123"
    When the user requests a token
    Then the login's API response status code should be 401
    And the login's API response reason should be "Bad credentials"
    But the login's API response status code is 200
...
# Scenario: Attempt to authenticate with empty credentials
#   Given this is a negative test case
#   And the user has username "" and password ""
#   When the user requests a token
#   Then the login's API response status code should be 400
#   And the login's API response reason should be "Bad request"
...
#   Given this is a negative test case
#   And the user has username "admin" and password "verylongpasswordthatexceedslimits123456789012345678901234567890"
#   When the user requests a token
#   Then the login's API response status code should be 400
#   And the login's API response reason should be "Bad request"